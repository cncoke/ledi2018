 ...
 */
package io.yc.library.httpokgo.request.base;
...
import io.yc.library.httpokgo.adapter.AdapterParam;
import io.yc.library.httpokgo.adapter.CacheCall;
import io.yc.library.httpokgo.adapter.Call;
import io.yc.library.httpokgo.adapter.CallAdapter;
import io.yc.library.httpokgo.cache.CacheEntity;
import io.yc.library.httpokgo.cache.CacheMode;
import io.yc.library.httpokgo.cache.policy.CachePolicy;
import io.yc.library.httpokgo.callback.Callback;
import io.yc.library.httpokgo.convert.Converter;
import io.yc.library.httpokgo.model.HttpHeaders;
import io.yc.library.httpokgo.model.HttpMethod;
...
//        baseUrl = url;
        HttpRequestUtils go = HttpRequestUtils.getInstance();
        baseUrl = go.getBaseurl();
    ...
    public R cacheTime(long cacheTime) {
        ...
        if (cacheTime > 1) {
            if (TextUtils.isEmpty(cacheKey)) cacheKey = HttpUtils.md5(url+params.toString());
            if (cacheMode == CacheMode.NO_CACHE || cacheMode == CacheMode.DEFAULT)
    ...
    public okhttp3.Call getRawCall() {
        ...
        if (client == null) client = HttpRequestUtils.getInstance().getOkHttpClient();
